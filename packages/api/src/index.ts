...
import {
  ...
  transformMovieIntoTextEmbedding,
} from "./utils";
...
const routes = app
  ...
  .post(
    "/movie/:id/interaction",
    zValidator(
      "json",
      z.object({
        timeAdded: z
          .date()
          .or(z.string().transform((d) => new Date(d)))
          .or(z.null())
          .optional(),
        timeFavorited: z
          .date()
  ...
  .get(
    ...
    async (c) => {
      ...
      const dirs = await db
        .select()
        .from(directors)
        .where(like(directors.name, `%${name}%`));
      return c.json(dirs);
    },
  )
  ...
  .get(
    "/movies/:ids",
    zValidator("param", z.object({ ids: z.string() })),
    async (c) => {
      const db = createDb(c.env.DB);
  ...
  .get(
    "/movies/year/:year",
    zValidator("param", z.object({ year: z.string().regex(/\d{4}/) })),
    async (c) => {
      const db = createDb(c.env.DB);
  ...
  .get(
    "/movies/genre/:genre",
    zValidator("param", z.object({ genre: z.string() })),
    async (c) => {
      const db = createDb(c.env.DB, { logger: true });
  ...
  .post(
    ...
    zValidator(
      "json",
      oauthAccountSchema.omit({ userId: true }).extend({
      ...
      if (existingUser) {
        ...
        setCookie(c, sessionCookie.name, sessionCookie.value, {
          ...
          sameSite: sessionCookie.attributes.sameSite
            ? capitalizeFirstLetter(sessionCookie.attributes.sameSite)
            : undefined,
        });
